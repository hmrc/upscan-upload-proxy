  ...
  "An XML error message" should:
    "when well-formed and having all fields populated" should:
      ...
      "be parseable into fields" in:
        toFields(key = "a-key", xmlMessage) should contain theSameElementsAs
          Seq(
            "key"            -> "a-key",
            "errorCode"      -> "NoSuchKey",
      ...
      "be translatable into JSON" in:
        toJson(key = "a-key", xmlMessage) shouldBe
          Json.parse:
            """{
            | "key": "a-key",
            | "errorCode": "NoSuchKey",
    ...
    "when not well-formed" should:
      ...
      "be silently ignored when translating into JSON" in:
        toJson(key = "a-key", xmlMessage) shouldBe
          Json.parse("""{"key": "a-key"}""")

    "be creatable from a plain message" in:
      toFields(key = "a-key", toXmlErrorBody("a plain message")) should contain theSameElementsAs
        Seq(
          "key"          -> "a-key",
