...
object ErrorActionParser:
  ...
  def parser(parser: PlayBodyParsers)(using ExecutionContext): BodyParser[ErrorAction] =
  ...
  private def fileIgnoreHandler(using ExecutionContext): Multipart.FilePartHandler[Unit] =
    fileInfo =>
  ...
  private def validateErrorAction(key: String, maybeRedirectUrl: Option[String]): Either[ParseError, ErrorAction] =
    maybeRedirectUrl
      .fold(Right(ErrorAction(None, key))):
        validateErrorActionRedirectUrlWithKey(_, key)
          .map(_ => ErrorAction(maybeRedirectUrl, key))
  ...
  private def handleParseError[A](multipartFormData: MultipartFormData[A])(parseError: ParseError): Result =
    val dataParts =
      multipartFormData.dataParts.view.filterKeys(TargetDataPartNames.contains)
