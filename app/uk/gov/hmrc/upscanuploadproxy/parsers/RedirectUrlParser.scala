...
object RedirectUrlParser  {
  def fileIgnoreHandler(implicit ec: ExecutionContext): Multipart.FilePartHandler[Unit] =
    fileInfo =>
      Accumulator(Sink.ignore)
        .map(_ => FilePart(fileInfo.partName, fileInfo.fileName, fileInfo.contentType, ()))

  def extractErrorActionRedirect(multiPartFormData: MultipartFormData[Unit]): Either[Result, String] =
    multiPartFormData.dataParts
      .get("error_action_redirect")
      .flatMap(_.headOption)
      .toRight(missingRedirectUrl)

  val missingRedirectUrl = Response.badRequest("Could not find error_action_redirect field in request")

  def redirectUrlParser(implicit ec: ExecutionContext, parser: PlayBodyParsers): BodyParser[String] =
    BodyParser { requestHeader =>
      parser.multipartFormData(fileIgnoreHandler)(requestHeader)
